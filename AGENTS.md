...
- English is the primary acquisition and default product language; Simplified Chinese is the only other product locale. Explicit user locale choices always override defaults, and public pages must not silently render summary content in a different language from the active route. Source media may still be detected and transcribed in other languages, but product UI, routes, and newly requested output locales are limited to `en` and `zh`.
...
- **Podcast Supply**: A bounded Railway cron syncs `config/podcast-sources.json`, discovers recent episodes, advances a durable historical cursor, and atomically submits `catalog_supply` tasks to `podcast_supply`. Each catalog task owns separate `en` and `zh` summary outputs over one shared transcript/evidence source. A bounded trusted private `trusted_codex` worker reuses the canonical pipeline with ChatGPT-managed Codex authentication. Only completed tasks that pass the database-owned public quality projection can become public.
...
5. **Model defaults SSOT**: edit provider default model names only in `config/llm-provider-defaults.json`. Run `cd frontend && npm run models:sync` to regenerate the bundled frontend mirror; build, lint and test hooks reject a stale mirror. Never read repository-relative model config files at function runtime.
...
14. **Minimalism Heuristic**: When multiple valid UI solutions exist, prefer the one with the least visual noise, the fewest transient states, and the smallest cognitive load while preserving clarity and speed. Do not treat animation quantity as a proxy for visual simplicity.
15. **Cloud-only Guardrail**: Development and tests may use localhost, but must keep the production Postgres, Supabase Auth/Realtime, queue semantics, and API contracts; do not introduce an alternative storage or event path. The ADR-approved trusted catalog runner changes only execution location and still uses the canonical Cloud queue and task state.
