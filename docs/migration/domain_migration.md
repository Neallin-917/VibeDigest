...
> [!IMPORTANT]
> **API 地址变更**: 生产公开浏览器 API `NEXT_PUBLIC_API_URL` 和 canonical backend URL `BACKEND_API_URL` 保持 `https://api.vibedigest.io`；Vercel server-only `BACKEND_ORIGIN_URL` 使用 Railway 直连域名 `https://vibedigest-production.up.railway.app`，避免服务端创建任务时经过 Cloudflare browser challenge。
