-- Billing period and monthly allowance are independent. Retain the existing
-- UTC calendar-month allowance boundary for both Basic and Pro.
alter table public.profiles
  add column billing_interval text check (billing_interval in ('monthly', 'annual')),
  add column cancel_at_period_end boolean;

-- Profiles contain paid entitlements only; clients may read their own row.
-- Supabase installations may grant table writes to authenticated by default.
...
begin
  ...
  if p_guest_id is not null then
    ...
    if v_profile.usage_reset_at <= now() then
      update public.profiles
         set usage_count = 0,
             usage_reset_at = (
